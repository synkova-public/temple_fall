...
bool Coin::init()
{
	...
	{
		if (!coin_texture.load_from_file(textures_path("coin.png"), true))
		{
	...
	float wr = coin_texture.width * 0.5f;
	...
	// Loading shaders
	if (!effect.load_from_file(shader_path("illuminated_texture.vs.glsl"), shader_path("illuminated_texture.fs.glsl")))
		return false;
...
vec2 Coin::get_bounding_box() const
{
	...
	return { std::fabs(physics.scale.x) * coin_texture.width, std::fabs(physics.scale.y) * coin_texture.height };
}

void Coin::set_speed(float sp) {
	motion.speed = sp;
}
