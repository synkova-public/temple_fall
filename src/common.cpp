...
float div(vec3 l, vec3 r) {
  return (dot(l,r) > 0 ? 1 : -1) * std::sqrt(dot(l,l) / dot(r,r));
}

bool practicallyZero(float a) {
  float tol = 0.01;
  return abs(a) < tol;
}
bool practicallyZero(vec3 a) {
  return practicallyZero(a.x) && practicallyZero(a.y) && practicallyZero(a.z);
}
bool intersection(vec2 a, vec2 b, vec2 c, vec2 d) {
  vec3 p = {a.x, a.y, 0};
  vec3 q = {c.x, c.y, 0};
  vec3 r = {b.x - a.x, b.y - a.y, 0};
  vec3 s = {d.x - c.x, d.y - c.y, 0};
  vec3 skew = cross(r, s);
  float t = div(cross(sub(q, p), s), skew);
  float u = div(cross(sub(q, p), r), skew);
  if (practicallyZero(skew)){
    return practicallyZero(cross(sub(q, p), r));
  } else {
    return (0 <= t <= 1 && 0 <= u <= 1);
  }
...
bool Texture::load_from_file(const char* path, bool should_clamp)
{
	if (path == nullptr) 
		return false;
